# Contributing Guidelines (Training Repositories)

## Notebook policy
- We version-control Jupyter notebooks (`.ipynb`) **with outputs** so learners can see evaluated results immediately.
- Each notebook is paired with a clean text representation (`.py` or `.md`) using [Jupytext].
- Execution counts and transient metadata are stripped automatically when you commit (to minimize unnecessary Git churn).
...
- strip execution counts and transient metadata but **keep outputs** (via nbstripout with `--keep-output`),
...
- If you need a **clean notebook without outputs** (e.g. for publication or reproducibility testing), run:
    
    ```bash
    jupyter nbconvert --clear-output --inplace notebook.ipynb
    ```
    
    and export that version separately (not committed here).
